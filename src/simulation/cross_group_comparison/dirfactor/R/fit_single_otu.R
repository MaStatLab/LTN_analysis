...
i=as.numeric(argv[2])
r=as.numeric(argv[3])
n_or_a=as.numeric(argv[4])
niter=as.numeric(argv[5]) # default in their code is 1000, and they used 1e5 in their simulation
...
resdir=paste0(WORK_DIR,'/cache/cross_group_comparison/single_otu/dirfactor/')
system(paste0('mkdir -p ',resdir,'/gibbs'))
system(paste0('mkdir -p ',resdir,'/teststat'))
data1dir=paste0(WORK_DIR,"/cache/ps_sim.RData")
data2dir=paste0(WORK_DIR,"/cache/cross_group_comparison/single_otu/")
