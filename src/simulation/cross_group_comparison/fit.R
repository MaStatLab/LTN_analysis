...
if (!file.exists(paste0(cachedir,'/pjap/',filenam))){
  ...
  SEED=1
  st<-system.time(t<-try(gibbs<-gibbs_crossgroup(N=N,p=p,g=g,r=r,YL=YL,Y=Y,Xtest=Xtest,Xadjust=Xadjust,grouplabel=grouplabel,niter=niter,adjust=adjust,reff=T,reffcov=reffcov,SEED=SEED,save_alpha_only=save_alpha_only,gprior_m=gm,pnull=pnull,a1a2='none',lambda_fixed=lambda)))
