...
cnt=readRDS(paste0(cachedir,'sim',SEED,'.RData'))$cnt
cnt[cnt==0]=0.5
coat_est=coat(cnt)$sigma
saveRDS(coat_est,paste0(resultdir,'est',SEED,'.RData'))
#----------------------ilr------------------------------------
...
for (modelCov in 1:3){
  ...
  clrtrue=readRDS(paste0(cachedir,modelcov,'clrcov_',i,'.RData'))
  cnt=readRDS(paste0(cachedir,modelcov,'cnt_',i,'.RData'))
  cnt[cnt==0]=0.5
  coat_est=coat(cnt)$sigma
  saveRDS(coat_est,paste0(resultdir,modelcov,'_est',i,'.RData'))
  }